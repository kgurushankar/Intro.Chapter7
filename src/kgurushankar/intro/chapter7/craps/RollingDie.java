...
public class RollingDie extends Die {
	private static final double slowdown = 0.97, speedFactor = 0.04, speedLimit = 2.0;
	...
	public void avoidCollision(RollingDie other) {
		...
		while (Math.abs(xCenter - other.xCenter) < dieSize && Math.abs(yCenter - other.yCenter) < dieSize)
			move();
	}
	...
	private void drawDots(Graphics g, int x, int y, int numDots) {
		...
		int step = dieSize / 8;
		int[] X = { x + step - 1, x + 3 * step, x + 5 * step + 1 };
		int[] Y = { y + step - 1, y + 3 * step, y + 5 * step + 1 };
		/*
		 * int x1 = x + step - 1; int x2 = x + 3 * step; int x3 = x + 5 * step +
		 * 1; int y1 = y + step - 1; int y2 = y + 3 * step; int y3 = y + 5 *
		 * step + 1;
		 */
		switch (numDots) {
		case 1: {
			g.fillOval(X[1], Y[1], dotSize, dotSize);
			break;
		}
		case 2: {
			g.fillOval(X[0], Y[0], dotSize, dotSize);
		...
		case 4: {
			g.fillOval(X[0], Y[0], dotSize, dotSize);
			g.fillOval(X[2], Y[0], dotSize, dotSize);
			g.fillOval(X[0], Y[2], dotSize, dotSize);
			g.fillOval(X[2], Y[2], dotSize, dotSize);
			break;
		}
		case 5: {
		...
		case 6: {
			g.fillOval(X[0], Y[0], dotSize, dotSize);
			g.fillOval(X[0], Y[1], dotSize, dotSize);
			g.fillOval(X[0], Y[2], dotSize, dotSize);
			g.fillOval(X[2], Y[0], dotSize, dotSize);
			g.fillOval(X[2], Y[1], dotSize, dotSize);
			g.fillOval(X[2], Y[2], dotSize, dotSize);
			break;
		}
		}
